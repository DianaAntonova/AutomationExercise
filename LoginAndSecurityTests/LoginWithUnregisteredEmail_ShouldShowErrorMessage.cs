...
{
          public class LoginWithUnregisteredEmailTests
          {
                    ...
                    [Test]
                    public void LoginWithUnregisteredEmail_ShouldShowErrorMessage()
                    {
                              // Generate random unregistered email
                              string randomPart = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 6);
                              string unregisteredEmail = $"notregisteredmail{randomPart}@test.bg";

                              // Enter unregistered email
                              driver.FindElement(By.XPath("//input[@data-qa='login-email']"))
                                    .SendKeys(unregisteredEmail);

                              // Enter any password
                              driver.FindElement(By.XPath("//input[@data-qa='login-password']"))
                                    .SendKeys("SomePassword123");
                              ...
                              Assert.That(
                                  ...
                                  $"Expected error message was not displayed for unregistered email. Email used: {unregisteredEmail}"
                              );
                    }
          }
}
